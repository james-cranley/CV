...
def parse_and_format_bib(
    ...
    import re
    # Wrap bare-word month values (e.g., `month=June`) in braces so the bibtex
    # parser doesn't try to resolve them as string macros. Doesn't touch values
    # already wrapped in `{...}` or `"..."`.
    input_bib = re.sub(
        r"(month\s*=\s*)([A-Za-z]+)(\s*[,}])",
        r"\1{\2}\3",
        input_bib,
    )
    parser = bp.bparser.BibTexParser(common_strings=True)
    parser.ignore_nonstandard_types = False
    db = bp.loads(input_bib, parser=parser)
